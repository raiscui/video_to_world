...
## [2026-03-22 02:10:54] [Session ID: 019d0ead-8f40-77b2-b6a3-2ed88d658c78] 笔记: 默认业务配置 3DGS 正式长跑已启动并完成首批稳定性观测

## 来源

### 来源1: 正式长跑训练日志
- 文件: `/tmp/video_to_world_formal_gs_longrun_20260322_020615.log`
- 输出目录:
...
- 关键事实:
  - 使用的是 `run_reconstruction fast` 对应的业务默认 GS 配置:
    - `renderer=3dgs`
    - `num_iters=10000`
    - `lpips_weight=0.2`
    - `auto_eval=true`
  - 已成功越过:
    - aligned point cloud 加载
    - valid pixel indices 计算
    - normals 估计
    - KNN 距离统计
    - LPIPS(VGG) 初始化
  - 训练已进入正式迭代,当前已推进到 200+ iter,loss 从约 `0.17456 -> 0.11088`。

### 来源2: GPU 周期采样日志
- 文件: `/tmp/video_to_world_formal_gs_longrun_20260322_020604_gpu.log`
- 关键事实:
  - 训练启动后的多轮采样里,显存稳定在约 `28373 / 49140 MiB`。
  - 目前 compute app 只有该训练进程,未观察到跨进程争抢显存。
  - 连续多个 30s 采样点未见显存持续爬升。

## 综合发现

### 已验证结论
- 这台 `49 GB` 显存的 RTX 6000 Ada 至少已经能够稳定拉起“默认业务配置”的 3DGS 正式长跑。
- 在正式长跑启动后的前几分钟里,没有出现“刚进入训练就 OOM”或“显存持续线性上涨”的现象。
- 因此,对 GS 训练这一段而言,当前动态证据进一步支持“不是简单的卡显存不够”。

### 仍待后续验证
- 默认 `10000 iter + auto eval` 的完整最终收尾尚在进行中,当前还没有终局证据。
## [2026-03-22 02:44:26] [Session ID: 019d0ead-8f40-77b2-b6a3-2ed88d658c78] 笔记: 当前 GS checkpoint 只够做评估/导出,不支持正式无缝续跑训练

## 来源

### 来源1: `configs/stage3_gs.py`
- 要点:
  - 当前 `GSConfig` 没有 `resume` / `checkpoint_path` / `start_iter` 之类的正式恢复训练参数。

### 来源2: `train_gs.py`
- 要点:
  - 训练循环固定是 `for it in range(config.num_iters)` 从 0 开始。
  - checkpoint 保存语句是:
    - `torch.save(model.state_dict(), ckpt_path)`
  - 最终模型保存语句是:
    - `torch.save(model.state_dict(), final_path)`
  - 代码里没有从 GS checkpoint 恢复 model / optimizer / scheduler / current_iter 的逻辑。

### 来源3: 动态检查 `checkpoint_000149.pt`
- 要点:
  - 文件载入后类型是 `collections.OrderedDict`
  - 内容是参数张量字典,例如 `canonical_points`、`sh_dc`、`sh_rest`
  - 没有看到 `optimizer`、`scheduler`、`iter` 这类恢复训练必须的状态

## 综合发现

### 已验证结论
- 当前仓库的 GS checkpoint **可以**用于:
  - `eval_gs`
  - `view_checkpoint`
  - `export_checkpoint_to_ply`
- 但它**不能**直接作为“正式无缝续跑训练”的输入。
- 如果 10000 iter 正常结束,以后最多是“拿最终权重继续做评估/导出”; 不是“原地从 10000 接着优化到 20000”。
- 如果想支持正式续跑,至少需要补:
  - resume CLI 参数
  - model / optimizer / scheduler / current_iter 保存与恢复
  - 最好再加 RNG state 保存

### 当前运行的实际风险
- 当前正式长跑配置 `save_every=5000`。
- 也就是说,如果在 5000 iter 之前人工停掉,当前这轮连一个可用 GS checkpoint 都还没有落盘。
- 即使停在 5000 或 10000 之后已经有 checkpoint,当前代码也仍然不能“无缝继续训练”,只能作为后续改造 resume 的起点材料。
## [2026-03-22 04:04:35] [Session ID: 019d0ead-8f40-77b2-b6a3-2ed88d658c78] 笔记: `add-gs-training-resume` 已通过 openspec-ff-change 推进到 apply-ready

## 来源

### 来源1: OpenSpec artifact status
- 要点:
  - `proposal`, `specs`, `design`, `tasks` 均已完成
  - `openspec status --change add-gs-training-resume --json` 返回 `isComplete: true`

### 来源2: OpenSpec validation
- 要点:
  - `openspec validate add-gs-training-resume` 已通过

## 综合发现

### 已验证结论
- `add-gs-training-resume` 已经具备实现前所需的全部 spec-driven artifacts。
- 当前 change 已处于 apply-ready 状态,下一步可以直接进入实现。
- 这次 artifacts 已明确覆盖:
  - proposal: 为什么需要 GS formal resume
  - specs: WHAT the system must do
  - design: HOW the checkpoint / resume / compatibility path should be built
