...
## [2026-03-22 02:00:30] [Session ID: 2479889] 任务名称: 用 HTTP/1.1 完成最后一次推送收口

### 任务内容
- 处理最后一个 `record publish completion` 提交的推送失败。
- 判断失败是否来自代理、认证还是 HTTP 传输层。
- 在不污染全局 Git 配置的前提下完成远程同步。

### 完成过程
- 先确认失败后的本地状态:
  - 本地仍领先远程 1 个提交: `9ff571c record publish completion`
- 再用去代理环境执行 `git ls-remote origin refs/heads/main`,确认远程可访问,且当时仍停在 `465421d...`。
- 基于这一证据,把假设收敛到 HTTP/2 传输层问题。
- 仅对单次命令使用 `git -c http.version=HTTP/1.1 push origin main`,成功把远程更新到 `9ff571ce99cd5692ccc504dc550644c7c9412ab7`。

### 总结感悟
- 去掉坏代理后,网络问题并不一定完全结束。HTTP/2 framing error 这种报错,更适合用“单次命令降级协议版本”的最小修法去验证。
- 能避免的就不要写全局配置。像 `http.version=HTTP/1.1` 这种 workaround,先放到单次命令里最稳。
